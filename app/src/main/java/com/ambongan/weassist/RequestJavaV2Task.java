...
public class RequestJavaV2Task extends AsyncTask<Void, Void, DetectIntentResponse> {
        ...
        try{
            DetectIntentRequest detectIntentRequest =
                    DetectIntentRequest.newBuilder()
                            ...
                            .build();
            return (DetectIntentResponse) sessionsClient.detectIntent(detectIntentRequest);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    protected void onPostExecute(DetectIntentResponse response) {
        ((MainActivity) activity).callbackV2((com.google.cloud.dialogflow.v2.DetectIntentResponse) response);
    }
}